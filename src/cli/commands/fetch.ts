...
export class FetchCommand extends Command {
  ...
  static override usage = Command.Usage({
    ...
    examples: [
      ...
      [
        ...
        "stocker fetch AAPL MSFT GOOGL --parallel 3",
      ],
    ],
  });
  ...
  async execute() {
    ...
    if (this.verbose) {
      console.log(
        `Fetching ${tickers.length} ticker(s) with parallel limit: ${parallelLimit}`
      );
    }
    ...
    for (const ticker of tickers) {
      try {
        ...
        if (exists && !this.update && !this.force) {
          ...
          if (metadata) {
            console.log(
              `${normalizedTicker}: Already exists (last update: ${metadata.lastUpdate}). Use --update or --force to fetch.`
            );
            continue;
          }
        }
        ...
        if (this.update && exists) {
          const lastUpdate = await stocker.storage.getLastUpdate(
            normalizedTicker
          );
          if (lastUpdate) {
            startDate = format(
              new Date(lastUpdate.getTime() + 24 * 60 * 60 * 1000),
              "yyyy-MM-dd"
            );
            if (this.verbose) {
        ...
        if (metadata) {
          console.log(
            `${normalizedTicker}: Successfully fetched (${metadata.recordCount} records, ${metadata.firstTradeDate} to ${metadata.lastUpdate})`
          );
        }
      } catch (error) {
        const errorMessage =
