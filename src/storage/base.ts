...
export class DuckDBBase {
  ...
  async writeParquet(data: any[], outputPath: string, schema?: Record<string, string>): Promise<void> {
    if (!this.db) throw new Error('Database not initialized');
  ...
  async mergeParquet(
    ...
    if (existsSync(existingPath)) {
      ...
      const schemaQuery = await this.query<{column_name: string, column_type: string}>(
        `DESCRIBE ${tempView}`
      );
