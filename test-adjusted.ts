...
async function testAdjustedPrices() {
  ...
  try {
    ...
    data.forEach(day => {
      console.log(
        `${day.date} | ${day.close.toFixed(2).padStart(9)} | ${day.adjClose.toFixed(2).padStart(9)} | ${day.volume.toLocaleString().padStart(10)} | ${day.adjVolume.toLocaleString().padStart(10)}`
      );
    });
    ...
    if (beforeSplit && afterSplit) {
      ...
      console.log(`\nAdjustment factor: ${adjFactor.toFixed(4)} (indicates ${1/adjFactor}:1 split)`);
    }
